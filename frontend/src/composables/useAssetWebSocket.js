...
const globalStatus = ref('DISCONNECTED');
const globalReconnectCount = ref(0);
let triggerReconnectFn = null;
/**
 ...
 */
export function useAssetWebSocket(assetId) {
    ...
    function handleReconnectFailure() {
        if (activeToastId)
            return; // Avoid duplicate toast alerts
    ...
    const { connectionStatus, reconnectCount, reconnect, disconnect } = useWebSocketManager(assetId, handleMessage, handleReconnectFailure);
