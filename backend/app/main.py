...
async def lifespan(app_inst: FastAPI) -> AsyncIterator[None]:
    """
    Lifespan event manager starting/stopping database background simulation processes.
    """
    from backend.app.services.market_data import background_update_loop
    task = asyncio.create_task(background_update_loop())
    log_event(logging.INFO, "backend_started", details="In-memory update loops spawned.")
    yield
    task.cancel()
    try:
        await task
        ...
        pass
    log_event(logging.INFO, "backend_stopped", details="Background tasks cleanly cancelled.")
...
# Custom Middleware for request/response structured logging
@app.middleware("http")
...
async def http_exception_handler(request: Request, exc: StarletteHTTPException) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=exc.status_code,
        content={"error": "HTTP_EXCEPTION", "message": exc.detail, "status_code": exc.status_code}
    )
