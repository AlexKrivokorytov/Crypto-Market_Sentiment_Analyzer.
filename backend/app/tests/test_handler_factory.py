...
async def test_factory_bootstrap_registers_all_assets(
    factory: AssetHandlerFactory,
) -> None:
...
async def test_factory_len_matches_config(factory: AssetHandlerFactory) -> None:
    assert len(factory) == len(HANDLER_CONFIG)


async def test_factory_bootstrap_is_idempotent() -> None:
...
async def test_crypto_handlers_are_crypto_type(factory: AssetHandlerFactory) -> None:
...
async def test_stock_handlers_are_stock_type(factory: AssetHandlerFactory) -> None:
...
async def test_get_known_asset_returns_handler(factory: AssetHandlerFactory) -> None:
    handler = factory.get("BTC")
    assert handler.asset_id == "BTC"
    assert handler.name == "Bitcoin"


async def test_get_unknown_asset_raises_key_error(factory: AssetHandlerFactory) -> None:
    with pytest.raises(KeyError, match="XYZ"):
        factory.get("XYZ")


async def test_all_returns_list_of_handlers(factory: AssetHandlerFactory) -> None:
    handlers = factory.all()
    assert isinstance(handlers, list)
    assert len(handlers) == len(HANDLER_CONFIG)


async def test_iteration_over_factory(factory: AssetHandlerFactory) -> None:
...
async def test_seed_document_has_required_keys(factory: AssetHandlerFactory) -> None:
    required_keys = {
        "id",
        "name",
        "symbol",
...
async def test_seed_document_price_equals_base_price(
    factory: AssetHandlerFactory,
) -> None:
    for handler in factory.all():
        doc = handler.to_seed_document()
...
async def test_seed_document_change24h_is_zero(factory: AssetHandlerFactory) -> None:
    for handler in factory.all():
        doc = handler.to_seed_document()
        assert doc["change24h"] == 0.0


async def test_seed_document_sentiment_label_mapping(
    factory: AssetHandlerFactory,
) -> None:
...
async def test_unknown_handler_type_raises_value_error() -> None:
