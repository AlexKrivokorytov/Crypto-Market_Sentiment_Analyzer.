...
def test_vader_negations() -> None:
    ...
    res = analyze_sentiment_local("Bitcoin is not bullish today", "Fears rise")
...
def test_vader_intensifiers() -> None:
    ...
    res_base = analyze_sentiment_local("Solana is bullish", "Activity grows")
    res_strong = analyze_sentiment_local("Solana is very bullish", "Activity grows")
    assert abs(res_strong["sentimentScore"]) > abs(res_base["sentimentScore"])

    # "slightly bearish" should be weaker than just "bearish"
    res_base_bear = analyze_sentiment_local("Solana is bearish", "Activity falls")
    res_weak_bear = analyze_sentiment_local(
