...
async def test_fetch_onchain_metrics_fallbacks_and_limits(
    ...
    res_aapl = await market_data_provider.fetch_onchain_metrics("AAPL")
    assert res_aapl == {}

    res_btc = await market_data_provider.fetch_onchain_metrics("BTC")
    assert res_btc == {}

    # 2. Active crypto assets should return valid, bounded fallback structures on network failure
    res_eth = await market_data_provider.fetch_onchain_metrics("ETH")
    ...
    res_sol = await market_data_provider.fetch_onchain_metrics("SOL")
    ...
    res_ton = await market_data_provider.fetch_onchain_metrics("TON")
