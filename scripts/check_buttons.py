        ...
        for asset_text, aria_label in assets_found:
            ...
            if asset_code in test_assets:
                ...
                if "AI Analysis" in content:
                    print(
                        f"[SUCCESS] OpenRouter AI Analysis successfully responded for {asset_code}!"
                    )
                elif "Local Algorithm (VADER)" in content:
                    print(
                        f"[WARNING] Circuit Breaker fell back to VADER for {asset_code}. OpenRouter LLM might have failed/timed out."
                    )
                else:
                    print(
                        f"[INFO] No specific AI badge detected in the HTML for {asset_code}. Check UI rendering."
                    )
                ...
                if articles:
                    ...
                    print(
                        f"First article preview: {articles[0].inner_text()[:200].replace(chr(10), ' ')}..."
                    )
                else:
